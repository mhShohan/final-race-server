...
class FeeFormServices {
  ...
  async create(payload: IFeeForm, userId: string) {
    ...
    if (residentialFee) {
      ...
      residentialFee.totalResidentFee = differenceInCalendarMonths(new Date(residentialFee.to), new Date(residentialFee.from)) * residentialFee.fee
    }
  ...
  async readAll() {
    return await FeeForm.find().populate('departmentalFeeId').populate('residentialFeeId').populate('semesterFeeId')
